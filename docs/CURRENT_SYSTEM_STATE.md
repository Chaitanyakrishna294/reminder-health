...
- `public.caregiver_connections`: Many-to-many relationship linkages. Columns: `id` (UUID), `patient_profile_id`, `caregiver_profile_id`, `connection_status` (`PENDING`, `ACCEPTED`, `REJECTED`), `is_active`, `relationship_type`, `is_primary`, `can_view_medications`, `can_view_vault`, `can_view_reports`, `can_edit_medications`, `can_receive_escalations`, `expires_at`.
- `public.caregiver_info`: Legacy registry for Caregiver ID exchanges. Columns: `id`, `caregiver_id` (`CG` + 6 digits), `caregiver_chat_id`, `caregiver_name`, `patient_telegram_id` (deprecated for relationship tracking), `connection_status`, `is_active`.
...
- **Patients**: Can manage (ALL) their own profile, medications, reminder events, logs, and notifications.
- **Caregivers**:
