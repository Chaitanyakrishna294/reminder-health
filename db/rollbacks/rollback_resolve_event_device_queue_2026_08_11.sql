-- Rollback for migration_resolve_event_device_queue_2026_08_11.sql
--
-- Restores resolve_reminder_event to the definition from
-- migration_fix_resolve_invalid_scheduled_time.sql (the version live before 2026-08-11),
-- byte-for-byte in behaviour: both virtual-event guards gated on `p_event_id IS NULL`
-- again, and the row lookup back after them.
--
-- COST OF ROLLING BACK: the Android offline action queue loses two cases permanently —
-- any Taken/Skip that syncs after the medication's local day rolls over, and any that
-- syncs after reminder_times was edited. Both fail 5 times and are then dropped, so a
-- patient's recorded dose disappears with no trace on either side. Prefer fixing forward.
--
-- CREATE OR REPLACE, never DROP + CREATE — a DROP resets the ACL to PUBLIC EXECUTE and
-- this function carries no GRANT block of its own to restore it.
