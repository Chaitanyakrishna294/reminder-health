...
| # | Migration | Notes |
|---|-----------|-------|
| — | `migration_ui_mode_lock_2026_08_13.sql` | 2026-08-13. Adds `profiles.ui_mode_locked boolean NOT NULL DEFAULT false` — the elderly view lock, set from Settings only. On the profile rather than localStorage because it is a caregiver's decision about someone else's device and must survive a reinstall or a new phone. No new RLS policy: it rides the existing own-row UPDATE on `profiles` (stated because the instinct is to add one, and a second overlapping policy on profiles is how the caregiver dual-read happened). Has validation. **Until applied the lock still works per-device from localStorage but does not follow the user** — the client logs a warning and degrades rather than failing the toggle. |
| — | `migration_lookup_auth_guards_2026_08_13.sql` | 2026-08-13. **Apply AFTER the sweep.** Adds `auth.uid()` guards inside `lookup_profile_by_connect_code` and `lookup_caregiver_by_code` — two locks on every door (grant + body), per `_TEMPLATE_new_rpc.sql`. These two are the only functions in the schema that turn a guessable string into PII, so an ACL accident must not be the single point of failure. Verified safe first: the **only** call site of either is `settings-client-view.tsx:190/:197` inside the auth-gated `(dashboard)` group, and neither appears in `src/` (the bot) or any API route — so no service_role caller exists, which matters because service_role has no `auth.uid()`. Bodies otherwise unchanged. Has rollback + validation; the validation's probe runs both functions in the editor (no `auth.uid()` there) and expects `AUTH_REQUIRED`, which also proves the bodies compile. **Still open, noted in the file:** an *authenticated* user can still walk the code space, and guest accounts are one tap — route through `check_rate_limit` if that matters before launch. |
| — | `migration_rotate_connect_codes_2026_08_13.sql` | 2026-08-13. Pre-launch hygiene: re-issues every `profiles.connect_code`. **NOT IDEMPOTENT — run once**; re-running invalidates the code people just wrote down. Rotation breaks **nothing** structural, checked before writing: `caregiver_connections` keys on profile UUIDs and `invite_caregiver(uuid)` takes a UUID, so established AND pending connections survive, and Settings just renders the new code — no re-issue flow needed. What breaks is exactly what should: an unredeemed code someone wrote down or messaged. Leaves `caregiver_info.caregiver_id` (legacy CG-ID) alone — different identifier, load-bearing for the three legacy rows, and now covered by the same guard. Row-by-row, not set-based, because `gen_connect_code()` checks uniqueness against the pre-update snapshot inside a single statement; ends with an in-transaction assert that no profile is left null and all codes are distinct. No rollback file — the old codes are gone by design. |
| — | `migration_revoke_anon_execute_sweep_2026_08_13.sql` | 2026-08-13. **Apply this INSTEAD of the single-function one below — it supersedes it** (same fix, whole schema). The audit found **68** functions `anon` could execute. Two groups were live holes, not defence-in-depth: (a) `lookup_profile_by_connect_code` / `lookup_caregiver_by_code` are SECURITY DEFINER with **no auth check** and return a real name + profile UUID from a short code — brute-forceable with the key shipped in the APK; (b) `expire_stale_connection_requests` / `cleanup_expired_trash` / `cleanup_expired_link_codes` / `cleanup_rate_limits` take no args, check no caller, and UPDATE/DELETE — including wiping `rate_limits`, which defeats rate limiting. Sweeps BOTH revokes over every non-extension function, then re-grants `authenticated` and `service_role` by name. **Skips the 31 extension-owned pg_trgm functions** (`pg_depend deptype='e'`) — the catalog's trigram search needs them and `CREATE EXTENSION` would restore the grants anyway. Trigger functions are included and it is safe: Postgres does not check EXECUTE when firing a trigger. Has rollback + validation; validation checks 3-5 assert the web app, the bot and catalog search all still work. |
| — | `migration_revoke_anon_execute_2026_08_13.sql` | **Superseded by the sweep above — apply the sweep instead.** Kept for the record: it fixes only `correct_reminder_event`. | 2026-08-13. Fixes check 3 of `validation_past_day_correction_2026_08_12.sql`, which came back FAIL with `anon` holding EXECUTE on `correct_reminder_event`. **Both revokes** — `FROM PUBLIC` (a NULL `proacl` means Postgres's default of EXECUTE to PUBLIC, and every role is PUBLIC) **and** `FROM anon` (Supabase's `ALTER DEFAULT PRIVILEGES` adds a direct grant to new functions). Neither alone is sufficient, and `REVOKE … FROM anon` against PUBLIC-derived access is a **silent no-op** — which is why the 2026-07 sweep and the 2026-08-12 migration each touched this function and it stayed open. Re-grants `authenticated` + `service_role`. Has rollback + validation. **After applying, re-run BOTH validations** — the new one, and `validation_past_day_correction_2026_08_12.sql` whose check 3 should now read DONE. Row 5 of the new validation counts how many OTHER public functions `anon` can still execute; if it warns, run `db/audits/audit_function_execute_grants_2026_08_13.sql` for the list. |
