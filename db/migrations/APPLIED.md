...
| # | Migration | Notes |
|---|-----------|-------|
| — | `migration_vault_upload_limits_2026_08_13.sql` | 2026-08-13. **Health Vault resource-exhaustion limits: 5 files per user, 5 MB per file, images + PDF only.** Sets `file_size_limit`/`allowed_mime_types` on the `health-vault` bucket (enforced by the Storage API itself) and replaces the vault INSERT policy on `storage.objects` with one that also blocks guests and calls `public.vault_can_accept_upload()`. **Why the DB and not the form:** the browser uploads straight to Supabase Storage with the anon key — our Next server is not in that path, so every check in the upload modal is advice to whoever chooses to use the form. **Two functions:** `vault_object_count()` (STABLE, granted to `authenticated`, read by the UI's "N of 5 used" so the number shown and the number enforced are the same one) and `vault_can_accept_upload()` (granted to **nobody** — it takes a per-user `pg_advisory_xact_lock` first, because without it fifty parallel uploads all read count = 0 and all pass). Counts **storage objects, not `health_records` rows**: a direct API upload creates the former and not the latter, and a trashed record keeps its object until `cleanup_expired_trash` purges it — so **trash occupies a slot until permanently deleted**, stated in the UI rather than hidden. Also closes a hole the guest guards left open: `guard_guest_write` blocks a guest's `health_records` row but nothing stopped their storage object, so the bytes were stored and billed while being invisible in the app. **Existing users over 5 keep everything** — the rule is `count < 5` on INSERT only. **Behaviour change to expect:** .doc/.docx/.txt/.zip uploads stop being accepted (already-stored files still open). Section 5 is an optional, clearly-marked rider setting the same ceilings on the `avatars` bucket, found by the same audit; delete it before running if unwanted. **Check 5 of the validation is the critical one** — the SECURITY DEFINER owner must have BYPASSRLS or the policy recurses and every upload fails. Has rollback + validation, plus `db/scripts/verify-vault-limits.mjs`, which proves the refusals by calling the Storage REST API directly rather than through the UI. |
...
| `vault_object_count` / `vault_can_accept_upload` | `migration_vault_upload_limits_2026_08_13.sql` (**PENDING**) — the 5-file limit. `vault_object_count()` is granted to `authenticated` and read by the vault UI; `vault_can_accept_upload()` is granted to **nobody** and called only from the `storage.objects` INSERT policy, because it takes a per-user advisory lock |
