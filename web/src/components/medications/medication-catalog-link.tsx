...
export default function MedicationCatalogLink({ value, onChange }: MedicationCatalogLinkProps) {
  ...
  if (value) {
    return (
      <div className="mt-2 flex items-start justify-between gap-3 surface-sunk px-4 py-3">
    ...
    <div className="mt-2 space-y-2">
      <div className="flex items-center gap-2">
        <input
          ...
          autoFocus
          className="flex-1 surface-sunk px-3.5 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-primary/40"
        />
        <button
          type="button"
      ...
      {results.length > 0 && (
        <div className="max-h-56 overflow-y-auto rounded-[var(--r-card)] border border-border divide-y divide-border">
          {results.map((row) => (
            <button
              key={row.id}
              type="button"
